...
如果 Surge Request List 显示 `MITM failed`，这通常是 MITM 主机名匹配或 iOS 证书信任状态问题，不是脚本已经改写失败。请先确认：

1. iOS 已在 **设置 → 通用 → 关于本机 → 证书信任设置** 中对 Surge CA 开启「完全信任」。
2. Request List 里失败请求的 Host 必须在模块 `[MITM] hostname` 中；当前 Surge 模块包含 `gs-loc.apple.com`、`gs-loc-cn.apple.com`、`gsp-ssl.ls.apple.com`、`bluedot.is.autonavi.com`、`bluedot.is.autonavi.com.gds.alibabadns.com`。
